	...
	function generateConf() {
		...
		*/
		...
		if(isset($this->_globals) && is_array($this->_globals)){
			...
			$output .= "#include extensions_custom.conf\n";
			$output .= "\n;end of [globals]\n\n";
		}
        ...
        $last_base_tag = false;
          ...
          if (is_array($this->_exts[$section][$extension])) 
{

          	foreach (array_keys($this->_exts[$section][$extension]) as $keyidx => $idx) {
            ...
            if ($last_base_tag && $ext['basetag'] = 'n') {
              ...
              $last_base_tag = false;
            	...
            	if(count(array_keys($this->_exts[$section][$extension])) -1 == $keyidx){
...
// $output .=" exten => ". trim($extension) .",n,Verbose(Start QoS)\n";
// $output .=" exten => ". trim($extension) .",n,Set(CDR(rtcp_jitter)=\${CHANNEL(rtcp,all_jitter)})\n";
// $output .=" exten => ". trim($extension) .",n,Set(CDR(rtcp_loss)=\${CHANNEL(rtcp,all_loss)})\n";
// $output .=" exten => ". trim($extension) .",n,Set(CDR(rtcp_rtt)=\${CHANNEL(rtcp,all_rtt)})\n";
// $output .=" exten => ". trim($extension) .",n,Set(CDR(rtcp)=\${CHANNEL(rtcp,all)})\n";
// $output .=" exten => ". trim($extension) .",n,NoOp(End of QoS)\n";


							  	if('ext-local' == $section){
            		...
            		// if("hangup" == trim($extension)){

									// $output .= "exten => ". trim($extension) .",n,Set(CDR(hangup_cause)=\${HANGUPCAUSE})\n";

									$output .= "exten => ". trim($extension) .",n,Set(CHANNEL(hangup_handler_push)=record-hangupcause,s,1)\n";
									$output .= "exten => ". trim($extension) .",n,Set(IVR_CONTEXT : ${IVR_CONTEXT})\n";

									// $output .= "exten => ". trim($extension) .",n,Set(CDR(jitter)= \${CHANNEL(rtpqos,audio,rxjitter)})\n";
									// $output .= "exten => ". trim($extension) .",n,Set(CDR(rtpqos_audio_all)=\${CHANNEL(rtpqos,audio,all)})\n";
									// $output .= "exten => ". trim($extension) .",n,Set(CDR(useragent)=\${CHANNEL(useragent)})\n";



									// $output .= "exten => ". trim($extension) .",n,Set(PACKETS_RECEIVED=\${CHANNEL(rtpqos,audio,rxcount)=0})\n";
									// $output .= "exten => ". trim($extension) .",n,Set(PACKETS_SENT=\${CHANNEL(rtpqos,audio,txcount)=0})\n";
									// $output .= "exten => ". trim($extension) .",n,Set(RECV_PACKETS_LOST=\${CHANNEL(rtpqos,audio,rxploss)=0})\n";
									// $output .= "exten => ". trim($extension) .",n,Set(SENT_PACKETS_LOST=\${CHANNEL(rtpqos,audio,txploss)=0})\n";
									// $output .= "exten => ". trim($extension) .",n,Set(JITTER_RECV=\${MATH(\${MATH(\${RECV_PACKETS_LOST}/\${PACKETS_RECEIVED}),float)}*100,float)}); Calculating loss of Received packets in %\n";
									// $output .= "exten => ". trim($extension) .",n,Set(JITTER_SENT=\${MATH(\${MATH(\${SENT_PACKETS_LOST}/\${PACKETS_SENT}),float)}*100,float)}); Calculating loss of Sent packets in %\n";
									// $output .= "exten => ". trim($extension) .",n,Set(JITTER_RECV_ROUNDED=\${SHELL(printf \"%.2f\\n\" $(echo \"\${JITTER_RECV}\" | bc -l))}); Rounding float numbers to two decimals (13.2164 -> 13.22)\n";
									// $output .= "exten => ". trim($extension) .",n,Set(JITTER_SENT_ROUNDED=\${SHELL(printf \"%.2f\\n\" $(echo \"\${JITTER_SENT}\" | bc -l))}); Rounding float numbers to two decimals (13.2164 -> 13.22)\n";
									// $output .= "exten => ". trim($extension) .",n,Set(PING_MIN=\${SHELL(printf \"%.0f\\n\" $(echo \"\${MATH(\${CHANNEL(rtpqos,audio,minrtt)}*1000))}\" | bc -l))}); Rounding on int ping_min average value (53.499 -> 53)\n";
									// $output .= "exten => ". trim($extension) .",n,Set(PING_AVERAGE=\${SHELL(printf \"%.0f\\n\" $(echo \"\${MATH(\${CHANNEL(rtpqos,audio,rtt)}*1000))}\" | bc -l))}); Rounding on int ping average value (53.499 -> 53)\n";
									// $output .= "exten => ". trim($extension) .",n,Set(PING_MAX=\${SHELL(printf \"%.0f\\n\" \$(echo \"\${MATH(\${CHANNEL(rtpqos,audio,maxrtt)}*1000))}\" | bc -l))}); Rounding on int ping_max average value(53.". trim($extension) ." -> 54)\n";
									// $output .= "exten => ". trim($extension) .",n,NoOp(Local RECV Jitter: \${CHANNEL(rtpqos,audio,rxjitter)})\n";
									// $output .= "exten => ". trim($extension) .",n,NoOp(Local SENT Jitter: \${CHANNEL(rtpqos,audio,txjitter)})\n";
									// $output .= "exten => ". trim($extension) .",n,NoOp(Local Jitter Max: \${CHANNEL(rtpqos,audio,local_maxjitter)})\n";
									// $output .= "exten => ". trim($extension) .",n,NoOp(Remote Jitter Max: \${CHANNEL(rtpqos,audio,remote_maxjitter)})\n";
									// $output .= "exten => ". trim($extension) .",n,NoOp(ALL: \${CHANNEL(rtpqos,audio,all)})\n";
									// $output .= "exten => ". trim($extension) .",n,NoOp(Local Jitter Min: \${CHANNEL(rtpqos,audio,local_minjitter)})\n";
									// $output .= "exten => ". trim($extension) .",n,NoOp(Dial failed for some reason with DIALSTATUS = \${DIALSTATUS} and HANGUPCAUSE = \${HANGUPCAUSE})\n";
